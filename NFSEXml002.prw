...
If !lNFeDesc
	//INICIO TOTVS PARANA CENTRAL --- 24/06/2015
	Private cMsgCnd	    := ""
	Private cXNFse 		:= ""
	Private aPedSd2  	:= {}  //BUSCA PEDIDOS DA SD2 PARA ANALISAR SE EXISTE CONTRATO/MEDICAO AMARRADA
	cNatOper += Chr(13)+Chr(10) //Pula Linha
	...
	IF DBSEEK(XFILIAL("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)
		WHILE !EOF() .AND. XFILIAL("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA == ;
			SD2->D2_FILIAL+SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA
				AADD(aPedSd2,{SD2->D2_PEDIDO,SD2->D2_ITEMPV})
			dbSkip()
		ENDDO
	ENDIF                   
	
		
	//VERIFICA SE EXISTE CONTRATO/MEDICAO AMARRADO A NF/PEDIDO E ANALISA O CAMPO CN9->CN9_X_MSGNF PARA DESCRICAO DO CORPO DA NF
	IF LEN(aPedSd2) > 0
		dbSelectArea("SC5")
		dbSetOrder(1)
		dbGoTop()
		IF DBSEEK(XFILIAL("SC5")+aPedSd2[1][1])
			IF !EMPTY(SC5->C5_MDCONTR) //EXISTE CONTRATO/MEDICAO AMARRADO
				dbSelectArea("CND")
				dbSetOrder(4)
				dbGoTop()
				IF DBSEEK(XFILIAL("CND")+SC5->C5_MDNUMED)
					cMsgCnd := ALLTRIM(CND->CND_X_MSNF)
					While !Eof() .AND. XFILIAL("CND")+SC5->C5_MDNUMED == CND->CND_FILIAL+CND->CND_NUMMED
						dbSelectArea("CN9")
						dbSetOrder(1)
						dbGoTop()
						IF DBSEEK(XFILIAL("CN9")+CND->CND_CONTRA+CND->CND_REVISA)
							IF !EMPTY(CN9->CN9_X_CNT)
								cMsgCntr := GETMV("MV_X_MSCNT")
								cNatOper += ALLTRIM(cMsgCntr)+" "+ALLTRIM(CN9->CN9_X_CNT)
							ENDIF
						ENDIF 
				...
				ENDIF
			ENDIF
			//Compoe Msg complementar NFSe
			cXNFse := Alltrim(SC5->C5_X_MSGNF)					
		ENDIF
	ENDIF		

	//MENSAGEM CONTRATO
	IF !EMPTY(cMsgCnd)
		cNatOper += Chr(13)+Chr(10) + ALLTRIM(MemoLine(cMsgCnd,,1))
		nLinMemo := MLCount(cMsgCnd) 
		IF nLinMemo > 1
			FOR XW := 2 TO nLinMemo
				cNatOper += Chr(13)+Chr(10) //Pula Linha
				cNatOper += ALLTRIM(MemoLine(cMsgCnd,,XW))
			NEXT XW
		ENDIF		
	ENDIF
	...
	//MENSAGEM NF                                  
	IF !EMPTY(cXNFse)
		cNatOper += Chr(13)+Chr(10) + ALLTRIM(MemoLine(cXNFse,,1))
		nLinMemo := MLCount(cXNFse) 
		IF nLinMemo > 1
			FOR XW := 2 TO nLinMemo
				cNatOper += Chr(13)+Chr(10) //Pula Linha
				cNatOper += ALLTRIM(MemoLine(cXNFse,,XW))
			NEXT XW
		ENDIF		
	ENDIF
	//FIM TOTVS PARANA CENTRAL --- 24/06/2015
